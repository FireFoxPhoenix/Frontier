...
{"["}color = Серый] Круглый идентификатор: [/color] {$ glound}
{"["}color = серый] Продолжительность: [/color] {$ duration}
{"["}color = серый] ford: [/color] {$ for Kid}
{"["}color = Grey] Версия: [/color] {$ версия}
{"["}color = серый] двигатель: [/color] {$ engversion}
{"["}color = grey] тип хэш: [/color] {$ hash}
{"["}color = Grey] Comp Hash: [/color] {$ comphash}
