...
cmd-mapping-try-grid = Не удалось загрузить файл в качестве карты. Попытка загрузить файл в виде сетки ...
cmd-mapping-success-load-grid = Загруженная неинициализованная сетка из файла {$ path} на новую карту с id {$ mapid}.
cmd-mapping-hint-grid = [Сетка]
