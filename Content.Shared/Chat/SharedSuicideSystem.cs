...
public sealed class SharedSuicideSystem : EntitySystem
{
    ...
    [Dependency] private readonly ConsciousnessSystem _consciousness = default!; // Forge Change
    ...
    {
        if (TryComp<ConsciousnessComponent>(target, out var victimConsciousness)) // Forge-Change
        ...
        _damageableSystem.TryChangeDamage(target, appliedDamageSpecifier, true, origin: target, targetPart: TargetBodyPart.Chest);
    ...
    public void ApplyLethalDamage(Entity<DamageableComponent> target, ProtoId<DamageTypePrototype>? damageType)
    {
        ...
        _damageableSystem.TryChangeDamage(target, damage, true, origin: target, targetPart: TargetBodyPart.Chest); // Forge Change
        Dirty(target, target.Comp);
    }
